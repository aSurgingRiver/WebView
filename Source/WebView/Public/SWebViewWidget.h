...
class WEBVIEW_API SWebViewWidget
	: public SBaseBrowser
{
public:

	BASEBROWSER_PARAMS(SWebViewWidget)
	/**
	...
	//
	void SetDockTab(TWeakPtr<SDockTab> docktab);
public:
	/** Default constructor. */
	SWebViewWidget();
	~SWebViewWidget();
...
private:
	...
	void HandleOnTitleChanged(FString title);
	bool HandleOnBeforePopup(FString ,FString );
private:
	TSharedPtr<SBaseBrowser> m_browser;
	TWeakPtr<SDockTab> m_docktab;
	webview::FOnUrlChanged m_OnUrlChanged;
	webview::FOnTitleChanged m_OnTitleChanged;
};
